...
// MODE decides which other vars are required — TASK for `runner`, PR_NUMBER +
// REVIEW_FEEDBACK for `review-followup`. A discriminated union on MODE (not a
// flat optional field on each) so this is enforced *and* narrows env's
// TypeScript type: inside `if (env.MODE === Mode.RUNNER)`, `env.TASK` is
// `string`, not `string | undefined` cast away at every call site; on the
// review-followup branch TASK doesn't exist on the type at all, and vice
// versa for PR_NUMBER/REVIEW_FEEDBACK. See createFinalSchema below for how
// this plugs into createEnv.
...
// (`-e MODE=`) the same way emptyStringAsUndefined does for every other var.
if (!process.env.MODE) {
...
export const env = createEnv({
  server: {
    ...
    // would read via .describe(). The actual per-MODE requiredness and env's
    // exported type come from RunnerModeEnv/ReviewFollowUpModeEnv above, via
    // createFinalSchema below, not from these individual field schemas.
    TASK: z
      .string()
      .min(1)
      .optional()
  ...
  // without setting every required var or risking the process.exit(1) above.
  // Set by vitest.config.ts; never set this in a real run — it defeats the
  // whole point of validating configuration.
