...
/**
 * `execFileSync` (not `execSync`) on purpose — arguments are passed as an
 * array, with no shell interpolation. The `taskSummary` passed into
 * `commitAndOpenPR` may originate from TASK, which in Phase 5 of the rollout
 * plan is meant to be set from a GitHub comment body — i.e. from any user
 * able to comment on an issue. A string interpolated into
 * `git commit -m "..."` would be a command injection vector (quote escaping,
 * `$()`, backticks, etc.).
 */
 ...
 * generated bundle) — not ordinary source/test files, which routinely run
 * past 10k chars. A 314-line test file (~11k chars) is normal, so an 8k cap
 * defeated the point (it got annotated as truncated and re-Read anyway).
 */
 ...
 * without re-reading the files it already changed — each Read it can skip is
 * a saved turn (the whole reason this is full current content, not a diff).
 * We send the current committed content of every changed file rather than a
 * diff against main: the model is going to *edit* those files, so their
 * present state is what it needs, and a diff would make it re-Read them to
 ...
 * `refs/remotes/origin/main`), which a real CI run hit and a local test with
 * an actual local "main" branch didn't catch. Assumes
 * {@link checkoutExistingBranch} already ran, so working-tree == HEAD and
 * `git show HEAD:<path>` is the committed source. Per-file and total caps
 * keep a large branch from blowing up the prompt; anything truncated is
 * annotated so the model knows to Read it.
 */
 ...
 * {@link checkoutExistingBranch} already ran, so the workspace is already on
 * the right branch.
 */
